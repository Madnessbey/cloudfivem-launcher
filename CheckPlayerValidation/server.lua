...
local identifier = GetPlayerEndpoint(source)
	PerformHttpRequest('http://00000/JoinApi/Update.php', function(err, text, headers) end, 'POST', json.encode({steam = identifier, timestamp=os.date('%Y-%m-%d %H:%M:%S')}), { ['Content-Type'] = 'application/json' })
	table.insert(SteamPlayersLeft, GetPlayerEndpoint(source))
end)

AddEventHandler("playerConnecting", function(name, setReason, deferrals)
	deferrals.defer()
	local identifier = GetPlayerEndpoint(source)
	deferrals.update("Launcher kontrol ediliyor")
	if not isPlayerAuthorised(identifier) then
		setReason("Lütfen launcher üzerinden giriş yapınız.")
		print("(" .. identifier .. ") " .. name .. " launcher harici giris yapiyor")
		deferrals.done("Lütfen launcher üzerinden giriş yapınız!")
		CancelEvent()
		return
	else
		deferrals.done()
...
Citizen.CreateThread(function()
	...
	PerformHttpRequest("http:///000000/JoinApi/Read.php", function(err, LoggedInUsers, headers)
				...
				for key, value in pairs (SteamPlayersLeft) do
					if v.steam == value then
						...
						table.remove(SteamPlayersLeft, key)
							PerformHttpRequest('http:///00000/JoinApi/Delete.php', function(err, text, headers) end, 'POST', json.encode({steam = v.steam}), { ['Content-Type'] = 'application/json' })
						end
					end
				end
			end
